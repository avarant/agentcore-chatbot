export default function OidcDiscoveryPage() {
  return (
    <article className="prose prose-gray max-w-3xl">
      <h1>OIDC Discovery Setup</h1>
      <p>
        Agent77 verifies user JWTs by fetching your public keys via standard
        OIDC discovery. You only need to serve two static-ish JSON endpoints —
        no full OIDC provider required.
      </p>

      <h2>1. OpenID Configuration</h2>
      <p>
        Serve a JSON response at{" "}
        <code>{"https://<your-domain>/.well-known/openid-configuration"}</code>:
      </p>
  ...
  "subject_types_supported": ["public"]
      ...
      <p>
        The critical field is <code>jwks_uri</code> — Agent77 uses it to find
        your public keys. The <code>issuer</code> must match the{" "}
        <code>iss</code> claim in your JWTs.
      </p>

      <h2>2. JWKS Endpoint</h2>
      <p>
        Serve your public key(s) at the URL specified in <code>jwks_uri</code>:
      </p>
      <pre><code className="language-json">{`{
  "keys": [
    {
      ...
      <p>
        The <code>kid</code> must match the <code>kid</code> header in JWTs
      ...
      <h3>Python</h3>
      <pre><code className="language-python">{`# generate_keys.py — run once
...
print("Keys written to private.pem and public.pem")`}</code></pre>

      <h3>Node.js</h3>
      <pre><code className="language-javascript">{`// generate-keys.js — run once
...
console.log("Keys written to private.pem and public.pem");`}</code></pre>

      <h2>Django Example</h2>
      <pre><code className="language-python">{`# urls.py
...
def jwks(request):
    ...
    return JsonResponse({
      ...
      <pre><code className="language-javascript">{`// routes/oidc.js
const crypto = require("crypto");
const fs = require("fs");
...
module.exports = { openidConfiguration, jwks };`}</code></pre>
    </article>
  );
}
