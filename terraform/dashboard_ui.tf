...
locals {
  enable_dashboard_ui = var.enable_dashboard && var.enable_dashboard_ui ? 1 : 0
}
...
resource "aws_cloudfront_distribution" "dashboard" {
  ...
  origin {
    domain_name = replace(replace(aws_lambda_function_url.dashboard[0].function_url, "https://", ""), "/", "")
    origin_id   = "api"
  ...
  default_cache_behavior {
    ...
    # SPA: rewrite non-file paths to .html
    function_association {
      event_type   = "viewer-request"
